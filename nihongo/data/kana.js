// Hiragana & Katakana datasets — base (gojūon), dakuten/handakuten, and yōon combos.
// Each entry: { k: kana, r: romaji }. Empty slots use null to keep grid alignment.
...
export const KANA_SETS = {
  hiragana: { base: HIRAGANA_BASE, dakuten: HIRAGANA_DAKUTEN, yoon: HIRAGANA_YOON },
  katakana: { base: KATAKANA_BASE, dakuten: KATAKANA_DAKUTEN, yoon: KATAKANA_YOON },
};

// Flat list (no nulls) — handy for counts and quizzes.
export function flatKana(script) {
  const s = KANA_SETS[script];
  return [...s.base, ...s.dakuten, ...s.yoon].filter(Boolean);
}
