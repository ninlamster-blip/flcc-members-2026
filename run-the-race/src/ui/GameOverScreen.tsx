...
export default function GameOverScreen() {
  const { score, highScore, distance, startGame, reset } = useGameStore()
  const isNewBest = score >= highScore && score > 0

  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-gray-950 to-black text-white px-6">
      ...
      <div className="bg-white/5 rounded-2xl border border-white/10 px-8 py-5 mb-6 text-center w-64">
        <div className="text-3xl font-bold text-yellow-300 tabular-nums">{score.toLocaleString()}</div>
        ...
        {!isNewBest && highScore > 0 && (
          <div className="mt-2 text-xs text-white/30">Best: {highScore.toLocaleString()}</div>
        )}
      </div>
