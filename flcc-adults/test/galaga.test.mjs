...
/**
 * The engine is the kids edition's, character for character.
 *
 * The two apps share no code, so this file is a duplicate on the same terms as
 * the crossword's layout engine — and a duplicate with nothing holding it in
 * place drifts. Only the header comment is allowed to differ.
 */
test('the engine has not drifted from the kids edition’s', async () => {
  const { readFileSync } = await import('node:fs');
  const ours = readFileSync(new URL('../js/games/galaga.js', import.meta.url), 'utf8');
  const theirs = readFileSync(new URL('../../flcc-next/js/games/galaga.js', import.meta.url), 'utf8');
  const body = (source) => source.slice(source.indexOf('export const WIDTH'));
  assert.equal(body(ours), body(theirs), 'the two Galaga engines have diverged — fix the bug in both files');
});

test('this edition never shows the score', async () => {
  const { readFileSync } = await import('node:fs');
  const screen = readFileSync(new URL('../js/screens/galaga.js', import.meta.url), 'utf8')
    .replace(/\/\*[\s\S]*?\*\//g, '').replace(/(^|[^:])\/\/.*$/gm, '$1');
  assert.equal(/state\.score/.test(screen), false, 'the adult Galaga screen reads the score — this edition keeps none');
});
