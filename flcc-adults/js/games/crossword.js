...
// is what makes it testable, and what lets a puzzle be *dealt* rather than
// stored. The day chooses nine clues out of the bank; this builds the grid for
// them; nobody authors a coordinate, and there is no puzzle file to run out of.
//
// DUPLICATED ON PURPOSE. This is the kids and teens edition's layout engine,
// character for character below the header, because the two apps share no code
// and this one may not import from that one. It is a pure algorithm with no
// design in it, so it is the least dangerous kind of duplicate — but a
// duplicate all the same, and `test/crossword.test.mjs` compares the two files
// and fails when they drift. Fix a bug here and fix it there.
