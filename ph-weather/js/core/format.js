...
/** A Date, read on the Philippine clock. */
...
/** Rain, which is the number this app is mostly about. */
export function mm(value, { perHour = false } = {}) {
  if (!Number.isFinite(value)) return '—';
  const shown = value >= 10 ? Math.round(value) : Math.round(value * 10) / 10;
  return `${shown} mm${perHour ? '/h' : ''}`;
}
 ...
 * ("2026-09-07T14:00") because it was asked for Philippine time. Parsing them
 * as UTC would shift them eight hours, so they are read as the Manila instants
 * they already are. The Philippines keeps no daylight saving.
 */
