// The Bible itself.
//
// Scripture is not content. Everything under `content/` is the ministry's own
// writing and can be edited from the dashboard; everything under `bible/` is
// the text of Scripture, is committed once by `scripts/build-next-bible.mjs`,
// and no screen in this app can change a word of it. That distinction is the
// whole reason this module is separate from `content.js`.
//
// Three translations ship with the app, all public domain:
//
//   web  World English Bible      modern English, the default
//   bbe  Bible in Basic English   about a thousand words, for a younger reader
//   tgl  Ang Dating Biblia (1905) Tagalog, the language most FLCC families pray in
//
// A book is one file. Opening John downloads John, not a Bible, and the
// service worker keeps what has actually been read — so a child who read
// Mark on the church wifi can still read Mark on the bus.
...
// ── Names ──────────────────────────────────────────────────────────────────
//
// A child types "john", "1 jn", "Juan", "Mga Awit" or "psalm". All of them
