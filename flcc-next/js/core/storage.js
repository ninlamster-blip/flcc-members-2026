...
//
// Everything FLCC NEXT keeps lives under `next/v1/`, and `guard()` throws on
// anything else. That is what stops this app from ever reading or writing the
// FLCC Members app's keys, Shepherd's, or LAMP's — by construction rather than
// by good intentions.

export const NS = 'next/v1/';

export function guard(key) {
  if (typeof key !== 'string' || !key.startsWith(NS)) {
    throw new Error(`FLCC NEXT storage refuses "${key}" — keys must start with "${NS}"`);
  }
  return key;
}
...
export const KEYS = {
  user:     `${NS}user`,      // users
  progress: `${NS}progress`,  // user_progress + completions
  prayers:  `${NS}prayers`,   // prayer_requests (this device's own)
...
// time Me is opened, so there is no second copy to fall out of step with the
// first.
